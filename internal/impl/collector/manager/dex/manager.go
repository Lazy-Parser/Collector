...
func (m *ManagerDex) Run(ctx context.Context) error {
	...
	var pancakeswapV2Pairs []database.Pair
	...
	for _, pair := range allPairs {
		if pair.Pool == "pancakeswap" && pair.Label == "v2" {
			pancakeswapV2Pairs = append(pancakeswapV2Pairs, pair)
			fmt.Printf("%s/%s\n", pair.BaseToken.Name, pair.QuoteToken.Name)
		}
		// if pair.BaseToken.Name == "gorilla" {
		// 	pancakeswapV2Pairs = append(pancakeswapV2Pairs, pair)
		// 	fmt.Printf("%s/%s\n", pair.BaseToken.Name, pair.QuoteToken.Name)
		// }
	}
	fmt.Printf("TOTAL: %d", len(pancakeswapV2Pairs))

	// start all collectors. Hardcode, change
	for _, collector := range m.list {
		go startCollector(ctx, collector, pancakeswapV2Pairs, consumerChan)
	}
...
func startCollector(
	...
	if err != nil {
		return fmt.Errorf("Failed to init '%s' collector. %v", (*collector).Name(), err)
	}

	err = (*collector).Connect()
	if err != nil {
		return fmt.Errorf("Failed to conenct '%s' collector. %v", (*collector).Name(), err)
	}

	err = (*collector).Subscribe()
	if err != nil {
		return fmt.Errorf("Failed to subscribe '%s' collector. %v", (*collector).Name(), err)
	}
